...
// Edited version of the React-Input-Trigger npm package
...
var InputTrigger = (function (_Component) {
  ...
  function InputTrigger(args) {
    ...
    var _this = _possibleConstructorReturn(
      this,
      (InputTrigger.__proto__ || Object.getPrototypeOf(InputTrigger)).call(
        this,
        args
      )
    );
  ...
  _createClass(InputTrigger, [
    ...
    },
    {
      ...
      value: function handleTrigger(event) {
        ...
        if (!triggered) {
          console.log('Trigger Key', key === trigger.key);
          console.log('Trigger Which', which === trigger.keyCode);
          // if ((which === trigger.keyCode || key === trigger.key) && shiftKey === !!trigger.shiftKey && ctrlKey === !!trigger.ctrlKey && metaKey === !!trigger.metaKey) {
          if (
            (key === trigger.key || regX.test(textareaValue)) &&
          ...
          setTimeout(function () {
            onType(
              getHookObject('typing', _this2.element, triggerStartPosition)
            );
          }, 0);
        }
