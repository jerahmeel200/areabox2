...
export default class Post extends Component {
  ...
  saveEmoji = (emojis) => {
    ...
    //new code
    const chatRef = refD('chat/' + channel_id + '/' + post.key + '/reactions/');
    update(chatRef, { emojis }).then(function () {
      console.log('reaction saved on chat');
    });
  };
  ...
  getUSerAvatar = (post, usersAvatars) => {
    ...
    const path = 'static/img/pins_svg/';
    ...
    if (post.user == userLogedIn) {
      ...
      if (userMetadata.profile && userMetadata.profile.avatar)
        avatar = userMetadata.profile.avatar;
  ...
  getTimeStamp = (array, index) => {
    ...
    else return postDate.toLocaleDateString([], options);
  };

  getAvatar = (userName) => {
    const { usersAvatars, usersProfiles } = this.state;
    //console.log("getAvatar user", userName)
    ...
    //     usersAvatars[userName] = myProfile.avatar;
